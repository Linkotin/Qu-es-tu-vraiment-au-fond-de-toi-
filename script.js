...
const PROFILE_SOURCES = window.PROFILE_SOURCES;
...
function loadProfileScript(key, onLoaded, onError) {
  const src = PROFILE_SOURCES[key];
  if (!src) {
    if (onError) onError();
    return;
  }
  const existing = document.querySelector('script[data-profile-key="' + key + '"]');
  if (existing) {
  ...
  script.src = src;
  script.dataset.profileKey = key;
  script.onload = () => { if (onLoaded) onLoaded(); };
...
function renderMissingProfile(key) {
  resultTitleEl.textContent = "Profil en attente d’écriture";
  resultArchetypeEl.textContent = key;
  resultAnalysisEl.textContent =
    "Ce profil existe dans la structure, mais son texte n’a pas encore été écrit ou chargé. Tu peux le définir toi-même ou le laisser en suspens pour l’instant.";
  resultStrengthsEl.innerHTML = "";
  resultWeaknessesEl.innerHTML = "";
  resultGrowthEl.innerHTML = "";
